...
class phRETS
{
	...
	public function GetObject($resource, $type, $id, $photo_number = '*', $location = 0)
	{
		...
		// make request
		$result = $this->RETSRequest(
			$this->capability_url['GetObject'],
			array(
				'Resource' => $resource,
				'Type' => $type,
				'ID' => $send_id,
	...
	public function SearchQuery($resource, $class, $query = "", $optional_params = array())
	{
		...
		while ($continue_searching) {
			...
			$data = $xml->{'RETS-RESPONSE'}->{'Property'};
			foreach ($data as $property) {
				// split up DATA row on delimiter found earlier
				$this->search_data[$this->int_result_pointer]['data'][] = "{$property['ID']}";
	...
	public function SearchDetail($propertyId)
	{
		$this->reset_error_info();
		...
		$search_arguments['Query'] = "(ID=$propertyId)";
		$search_arguments['QueryType'] = "DMQL2";
		...
		$detail = $xml->{"RETS-RESPONSE"}->PropertyDetails;
	...
	public function GetAllLookupValues($resource)
	{
		...
		// make request
		$result = $this->RETSRequest(
			$this->capability_url['GetMetadata'],
			array(
